...
class Fish():
   ...
   def __init__(self, id=None, genes=()):
       ...
       if genes: # inheritance
           ...
           self.id = id
           self.discrete = rand.choices(range(NUMBER_OF_DISCRETE), weights=[5, 1, 3, 2], k=1)[0]
           # Set color
           if (self.agro / 101) > 1:
               g = 255
           else:
...
while running:
   ...
   if start_menu:
       ...
       for fish in ALL_FISH:
           ...
           if fish.id:
               if ((not fish.hunt and rand.randint(0, 8)) or not rand.randint(-170, int(0.7 * (fish.decisiveness // (fish.targ_count + 1))))):
                   fish.radar_and_lock()
